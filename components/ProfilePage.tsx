...
export default function ProfilePage() {
  ...
  const renderSidebarLinks = (onItemClick?: () => void) => (
    <div className="flex flex-col gap-1.5">
      ...
      <Link href={`/profile/${currentUserProfile?.username || currentUser?.id}`} className={profile && currentUser && profile.id === currentUser.id ? activeLinkStyle : inactiveLinkStyle} onClick={onItemClick}>
        <ProfileIcon />
        <span>Profile</span>
      </Link>
      ...
      <aside className="uget-sidebar">
        ...
        {currentUserProfile && (
          <div className="flex items-center gap-3 border-t border-gray-100 pt-4 mt-auto">
            <Link href={`/profile/${currentUserProfile?.username || currentUser?.id}`} className="w-10 h-10 rounded-full overflow-hidden flex-shrink-0" style={{ display: "block" }}>
